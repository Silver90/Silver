...
<
    ...
    fields = "fields"
    form = "theForm"
    field - root - el = "area" > < / formly - form >
    `)
    ;
...
the
scope
's "form" property to the given FormController if it has a value`, () => {
const el = compileAndDigest(
`
<
form
...
the
scope
's "form" property to its own FormController if it doesn'
t
have
a
value
...
should
warn
if there's no FormController to be assigned`, inject(($log) => {
compileAndDigest(
`
<
formly - form
...
the
field
's scope when using a different form root element`, () => {
scope.fields = [getNewField()];
const el = compileAndDigest(
`
<
...
<
    ...
    name = "parent" >
    < div
    ng - repeat = "forms in [1, 2] track by $index" >
    < formly - form
    model = "model"
    fields = "fields"
    bind - name = "$parent.$index + '_in_my_ng_repeat'" > < / formly - form >
    < / div >
    < / form >
    `)
    ;
...
the
child
's updateInitialValue and resetModel from the parent`, () => {
...
<
    ...
    model = "model"
    fields = "fields"
    hide - directive = "ng-show" > < / formly - form >
        `)
    ;
    const fieldNode = el[0].querySelector('.formly-field');
...
>
{
    ...
    model = "model"
    fields = "fields"
    track - by = "field.key" > < / formly - form >`;
...
the
field
's specified values are not unique`, () => {
...
function expectTrackBy(trackBy) {
    expect(el[0].innerHTML).to.contain(`field in fields
    track
    by
    ${trackBy}
`)
...
>
{
    ...
    to.throw(/^Formly Error: fieldTransform must return an array of fields/);
}
)
;
