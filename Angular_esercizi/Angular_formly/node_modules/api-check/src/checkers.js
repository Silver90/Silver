...
let shapeChecker = setupChecker(function shapeCheckerDefinition(val, name, location) {
    ...
    {
        shapePropError = checker(val[prop], prop, `${location}
        ${name}
    `,
        val
    )
...
shapeChecker.strict = setupChecker(function strictShapeCheckerDefinition(val, name, location) {
    ...
    if (extraProps.length) {
        ...
        have
        extra
        properties: ${t(extraProps.join('`, `'))}.` +
            `It
        is
        limited
        to
        ${t(allowedProperties.join('`, `'))}
    `
    )
        ;
    }
...
function getRequiredIfNotChecker(all, otherProps, propChecker) {
    ...
    ;
    const description = `specified
    ${ifProps}
    these
    are
    not
    specified: ${props} (otherwise
    it
    's optional)`;
    const shortType = `requiredIfNot$
    {
        all ? '.all' : ''
    }
    [
