...
>
    {
        expect(() = > getApiCheck({prefix: 'apiCheck1'})
    ).
        to.throw(
...
>
    {
        ...
        expect(() = > getApiCheck(null, {myChecker: myImproperChecker})
    ).
        to.throw(
...
any.optional
that
's in the middle of the arg list`, () => {
(function (a, b, c) {
...
custom
object
's name if it has no properties`, () => {
function Foo() {
}
const foo = new Foo();
...
>
{
    ...
    (function (a) {
        var message = apiCheckInstance(apiCheckInstance.number, a).message;
        expect(message).to.match(makeSpacedRegex(`you
        passed
        ${func.name}
...
>
{
    ...
    expect(message).to.match(makeSpacedRegex(`bar
    foo \\[circular ~\\]
    bar
    foo \\[circular\\]`))
    ;
...
(function (a, b) {
    expect(() = > apiCheckInstance.throw([apiCheckInstance.string, apiCheckInstance.bool], arguments)
    ).
    to.throw(error);
})();
})
...
>
    {
        ...
        expect(() = > apiCheckInstance(ipAddressChecker, args)
    ).
        to.throw();
    }
)
...
>
    {
        ...
        expect(() = > apiCheckInstance(api, 'foo')
    ).
        to.throw(makeSpacedRegex(
...
(function (a) {
    expect(() = > apiCheckInstance.throw(apiCheckInstance.number, a)
    ).
    to.throw(/argument.*?1.*?must.*?be.*?number/i);
})('a', 3);
})
;
...
describe('apiCheck api', () = > {
    ...
    (function (a) {
        expect(() = > apiCheckInstance(null, arguments)
        ).
        to.throw(/argument.*1.*must.*be.*typeOrArrayOf.*func\.withProperties/i);
    })('a');
})
;
it(
...
>
{
    (function (a) {
        var args = arguments;
        expect(() = > apiCheckInstance(true, args)
        ).
        to.throw(/argument.*1.*must.*be.*typeOrArrayOf.*func\.withProperties/i);
    })('a');
}
...
it('should prefix the error message', () = > {
    expect(getFailureMessage()).to.match(new RegExp(
`^${gPrefix}`))
;
})
;
...
expect(getFailureMessage({prefix})).to.match(new RegExp(
`^${gPrefix} ${prefix}`))
;
})
;
...
it('should suffix the error message', () = > {
    expect(getFailureMessage()).to.contain(
`${gSuffix}`)
;
})
;
...
expect(getFailureMessage({suffix})).to.contain(
`${suffix} ${gSuffix}`)
;
})
;
...
expect(getFailureMessage({urlSuffix})).to.contain(
`${docsBaseUrl}${urlSuffix}`)
;
})
;
...
>
{
    expect(() = > getFailureMessage({myProp: true})
).
    to.throw(/argument 3.*?cannot have extra properties.*?myProp/i);
}
)
;
        ...
        shape: {
            ...
            bar: {
                __apiCheckData: {
                    ...
                    missing: 'MISSING THIS FUNC.WITHPROPERTIES'
                },
                'func.withProperties': {
...
it('should throw the message when the second argument is truthy', () = > {
    expect(() = > apiCheckInstance.handleErrorMessage('message', true)
).
to.throw('message');
})
;
