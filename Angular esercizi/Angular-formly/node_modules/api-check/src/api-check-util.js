...
function list(arry, join, finalJoin) {
    ...
    return copy.join(join) + `${copy.length ? join + finalJoin : ''}
    ${last}
`;
}
...
function getError(name, location, checkerType) {
    ...
    return new Error(`${nAtL(name, location)}
    must
    be
    ${t(stringType)}
`)
...
function setupChecker(checker, properties, disabled) {
    ...
    if (!checker.displayName) {
        checker.displayName = `apiCheck
        ${t(checker.shortType || checker.type || checker.name)}
        type
        checker`;
    }
