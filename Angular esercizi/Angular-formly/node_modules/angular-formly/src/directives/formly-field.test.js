...
expect(() = > compileAndDigest()
).
to.throw(/extra.*properties.*extraProp/);
})
;
...
the
model
's value to the specified value if it is not defined`, () => {
compileAndDigest();
...
the
model
's value if the specified value is defined`, () => {
...
>
    {
        const template = `<
        div
        data - ng - model = "model[options.key]" > < / div >`;
        scope.fields = [getNewField({template: template})];
        compileAndDigest();
        expect(isolateScope.fc).to.exist;
        expect(field.formControl).to.exist;
...
describe(
`
with custom errorExistsAndShouldBeVisible
expression
`,
() =
>
...
>
    {
        ...
        with the $viewValue, $modelValue, and
        scope`,
        () =
    >
        {
...
>
{
    ...
    const fieldPre1 = sinon.spy(template = > `fieldPre1_$
    {
        template
    }`)
    ;
    const fieldPre2 = sinon.spy(template = > `fieldPre2_$
    {
        template
    }`)
    ;
    const fieldPost1 = sinon.spy(template = > `fieldPost1_$
    {
        template
    }`)
    ;
    const fieldPost2 = sinon.spy(template = > `fieldPost2_$
    {
        template
    }`)
    ;

    const formPre1 = sinon.spy(template = > `formPre1_$
    {
        template
    }`)
    ;
    const formPre2 = sinon.spy(template = > `formPre2_$
    {
        template
    }`)
    ;
    const formPost1 = sinon.spy(template = > `formPost1_$
    {
        template
    }`)
    ;
    const formPost2 = sinon.spy(template = > `formPost2_$
    {
        template
    }`)
    ;

    const globalPre1 = sinon.spy(template = > `globalPre1_$
    {
        template
    }`)
    ;
    const globalPre2 = sinon.spy(template = > `globalPre2_$
    {
        template
    }`)
    ;
    const globalPost1 = sinon.spy(template = > `globalPost1_$
    {
        template
    }`)
    ;
    const globalPost2 = sinon.spy(template = > `globalPost2_$
    {
        template
    }`)
    ;
