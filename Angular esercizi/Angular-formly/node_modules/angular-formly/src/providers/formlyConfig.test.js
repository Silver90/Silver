...
describe('(◞‸◟；) path', () = > {
    it('should throw an error when providing both a template and templateUrl', () = > {
        expect(() = > setterFn({template, templateUrl}, name)
).
to.throw(/`template` must be `ifNot\[templateUrl]`/i);
})
;
...
expect(() = > setterFn({template: 'no formly-transclude'})
).
to.throw(error);
})
;
...
expect(() = > setterFn({template, types: ['hi', 2, false, 'cool']})
).
to.throw(error);
})
;
...
extends
    ...
    without
    a
    template, templateUrl, or
    defaultOptions`,
    () =
>
    {
        expect(() = > setterFn({name: 'type3', extends: 'type2'})
    ).
        to.not.throw();
    }
)
...
extended
parent
's defaultOptions function and its own defaultOptions function`, () => {
setterFn([
...
extended
parent
's defaultOptions function when it doesn'
t
have
one
of
...
defaultOptions
with the parent
's defaultOptions object merged with the given args`, () => {
setterFn([
    {name, defaultOptions: parentDefaultOptions},
    {name: 'type2', extends: name, defaultOptions: childFn}
]);
...
the
child
's defaultOptions with the given options to the parent'
s
validateOptions
function
`,
...
the
child
's when there is a child controller function`, inject(($log, $http) => {
setterFn([
    {name, template, controller: parentFn},
...
function when
there
's no parent controller`, inject(($http) => {
setterFn([
    {name, template},
    {name: 'type2', extends: name, controller: childFn}
]);
...
describe('(◞‸◟；) path', () = > {
    it('should throw an error when the first argument is not an object or an array', () = > {
        expect(() = > setterFn('string')
).
to.throw(/must.*provide.*object.*array/);
expect(() = > setterFn(324)
).
to.throw(/must.*provide.*object.*array/);
expect(() = > setterFn(false)
).
to.throw(/must.*provide.*object.*array/);
})
;
...
it('should throw an error when a name is not provided', () = > {
    expect(() = > setterFn({templateUrl})
).
to.throw(/formlyConfig\.setType/);
})
;
...
>
{
    expect(() = > setterFn({name, template, templateUrl})
).
    to.throw(/formlyConfig\.setType/);
}
)
...
>
{
    expect(() = > setterFn({name, templateUrl, extra: true})
).
    to.throw(/formlyConfig\.setType/);
}
)
