...
`)
    ;
    expect(resultEl.attr(`ng -
    ${name}
`)).
    to.eq(`options.templateOptions['${name}']`)
    ;
}
)
;
...
;
manipulate();
expect(resultEl.attr(name)).to.eq(
`
{
    {
        options.templateOptions['${name}']
    }
}
`)
;
...
to
maxlength
if it's not in expressionProperties`, () => {
